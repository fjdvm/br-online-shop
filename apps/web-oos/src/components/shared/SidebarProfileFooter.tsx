        ...
        <DropdownMenuTrigger asChild>
          <div className="flex items-center gap-2 p-2 rounded-xl hover:bg-sidebar-accent transition-colors cursor-pointer group">
            <div className="relative shrink-0">
              ...
              <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-sidebar" title="Online" />
            </div>
            <div className="flex-1 overflow-hidden space-y-0.5">
              <p className="text-sm font-bold truncate text-sidebar-foreground leading-tight">{name}</p>
              <p className="text-xs font-medium text-muted-foreground truncate leading-tight">{email}</p>
            </div>
        ...
        <DropdownMenuContent className="w-64 bg-popover border-border text-popover-foreground z-[99999] shadow-xl p-2" side="top" align="start">
          ...
          <DropdownMenuItem
            onClick={async () => {
              ...
              window.location.href = `${process.env.NEXT_PUBLIC_AUTH_ISSUER || "https://localhost:5001/"}connect/logout?post_logout_redirect_uri=${process.env.NEXT_PUBLIC_OOS_URL || "https://localhost:3004/"}`;
            }}
