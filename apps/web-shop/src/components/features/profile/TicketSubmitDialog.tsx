...
const TICKET_SUBJECT_MAX = 150;
const TICKET_DESCRIPTION_MAX = 4000;
...
export function TicketSubmitDialog({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const trimmedSubject = trimField(subject);
    const trimmedDescription = trimField(description);
    if (!trimmedSubject) {
      setErrorMessage("Subject is required.");
      return;
    }
    if (trimmedSubject.length > TICKET_SUBJECT_MAX) {
      setErrorMessage(`Subject must be ${TICKET_SUBJECT_MAX} characters or less.`);
      return;
    }
    ...
    if (trimmedDescription.length > TICKET_DESCRIPTION_MAX) {
      setErrorMessage(`Description must be ${TICKET_DESCRIPTION_MAX} characters or less.`);
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <CharacterCount value={subject} max={TICKET_SUBJECT_MAX} />
            </div>
            <Input
              id="ticket-subject"
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <CharacterCount value={description} max={TICKET_DESCRIPTION_MAX} />
            </div>
            <Textarea
