...
public sealed class AuthServiceSignupWebhookTests : IDisposable
{
    ...
    [Fact]
    public async Task Register_relays_the_marketing_consent_answer_on_the_webhook()
    {
        await _authService.RegisterAsync(new RegisterRequest(
            "Opted In", "optedin@example.com", "Password123!", MarketingOptIn: true));

        var evt = _webhook.Sent.Single();
        evt.Data.MarketingOptIn.Should().BeTrue();

        var user = await _context.Users.SingleAsync(u => u.Email == "optedin@example.com");
        user.MarketingOptIn.Should().BeTrue();
    }

    [Fact]
    public async Task Register_relays_a_declined_marketing_consent_answer()
    {
        await _authService.RegisterAsync(new RegisterRequest(
            "Opted Out", "optedout@example.com", "Password123!", MarketingOptIn: false));

        var evt = _webhook.Sent.Single();
        evt.Data.MarketingOptIn.Should().BeFalse();
    }
